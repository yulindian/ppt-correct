...
Also inventory run-level formatting before replacing OCR text. A mixed-style text shape is any shape whose reference uses more than one visible color, weight, size, or font within the same sentence or paragraph. Whole-shape `text=`, `font=`, `bold=`, or `color=` edits may collapse runs; recreate the intended emphasis with run-level, `--find`, or character-range formatting. After the batch, read back runs for every mixed-style shape and every peer group touched by weight/color changes, then render: both the emphasized substrings and peer base style must match the reference. Shape-level aggregate color or boldness is insufficient.
...
Treat the cover as a high-salience typography checkpoint. For its main title, do not accept a font merely because it belongs to the requested category (for example, “行楷感”): render the closest practical candidates and compare stroke mass, glyph structure, total line width, and visual density with the PDF. For every title or label required to stay on one line, including cover metadata, use fixed sizing with `autoFit=none`, explicit margins, and at least 15% spare width (25% for decorative Chinese faces) inside the intended gap. A box that barely fits in one renderer is not stable; if supplied Office/WPS evidence disagrees, use that screenshot as the acceptance reference and audit peer titles.
